 ...
 * Vite/ES Module Compatible Version
 */
...
function highlightSubmit(step) {
    ...
    // The form submission is handled by the tutorial:actionCompleted event
    // dispatched from each page's form handler
}
...
function handleActionComplete(e) {
    // Only process if tutorial is active
    if (!isActive) {
        console.log('Tutorial not active, ignoring action complete event');
        return;
    }
    
    // Check if this is the correct step
    const expectedStep = currentStep + 1;
    if (e.detail?.step === expectedStep) {
        console.log(`Step ${e.detail.step} completed, advancing to next step`);
        showSuccess();
        
        // Check if this was the last step (step 5 - Insurance)
        if (currentStep >= CONFIG.steps.length - 1) {
            console.log('Final step completed, showing completion');
            setTimeout(() => {
                cleanup();
                showComplete();
            }, 1500);
        ...
        console.log(`Step mismatch: expected ${expectedStep}, got ${e.detail?.step}, currentStep: ${currentStep}`);
    }
}


// Public API
const VIMSYSTutorial = {
    start,
    next,
    prev,
    skip,
...
window.VIMSYSTutorial = VIMSYSTutorial;
