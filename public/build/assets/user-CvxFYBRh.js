import{g as l}from"./index-CB87Sc6I.js";import{S as f}from"./sweetalert2.esm.all-BoFtKDfH.js";var L;const v=((L=document.querySelector('meta[name="csrf-token"]'))==null?void 0:L.getAttribute("content"))||"";let m=[],d=[],u="cards",w="create";const e={totalUsers:document.getElementById("totalUsers"),activeUsers:document.getElementById("activeUsers"),inactiveUsers:document.getElementById("inactiveUsers"),cardView:document.getElementById("cardView"),tableView:document.getElementById("tableView"),tableBody:document.getElementById("tableBody"),filteredEmptyState:document.getElementById("filteredEmptyState"),statusFilter:document.getElementById("statusFilter"),userModal:document.getElementById("userModal"),modalOverlay:document.getElementById("modalOverlay"),modalTitle:document.getElementById("modalTitle"),submitBtn:document.getElementById("submitBtn"),userForm:document.getElementById("userForm"),formMethod:document.getElementById("formMethod"),viewOnlyGroup:document.getElementById("viewOnlyGroup"),createdDateDisplay:document.getElementById("createdDateDisplay"),firstname:document.getElementById("firstname"),middlename:document.getElementById("middlename"),lastname:document.getElementById("lastname"),username:document.getElementById("username"),email:document.getElementById("email"),password:document.getElementById("password"),avatar:document.getElementById("avatar"),type:document.getElementById("type"),status:document.getElementById("status"),permissions:document.getElementById("permissions"),credit:document.getElementById("credit"),office:document.getElementById("office")};function B(a){if(!a)return"N/A";const t=window.offices.find(s=>s.id==a);return t?t.office_name:"N/A"}document.addEventListener("DOMContentLoaded",function(){var a,t,s,i,o,n;g(),p(),(a=document.getElementById("loadDataBtn"))==null||a.addEventListener("click",p),(t=document.getElementById("create_new"))==null||t.addEventListener("click",openCreateModal),(s=document.getElementById("resetFilters"))==null||s.addEventListener("click",M),document.querySelectorAll(".toggle-btn").forEach(r=>{r.addEventListener("click",function(){u=this.dataset.view,document.querySelectorAll(".toggle-btn").forEach(c=>c.classList.remove("active")),this.classList.add("active"),e.cardView.style.display="none",e.tableView.style.display="none",e.cardView.classList.remove("active","cards-view","grid-view","list-view"),e.cardView.classList.add("users-grid"),u==="cards"?(e.cardView.classList.add("cards-view","active"),e.cardView.style.display="grid",y(d)):u==="grid"?(e.cardView.classList.add("grid-view","active"),e.cardView.style.display="grid",y(d)):u==="list"?(e.cardView.classList.add("list-view","active"),e.cardView.style.display="grid",y(d)):u==="table"&&(e.tableView.classList.add("active"),e.tableView.style.display="block",h(d))})}),(i=e.statusFilter)==null||i.addEventListener("change",function(){A(this.value)}),(o=e.userForm)==null||o.addEventListener("submit",function(r){r.preventDefault(),!e.submitBtn.disabled&&(e.submitBtn.disabled=!0,e.submitBtn.innerHTML='<i class="fas fa-spinner fa-spin"></i> Saving...',k())}),(n=e.modalOverlay)==null||n.addEventListener("click",closeUserModal)});function M(){e.statusFilter.value="all",A("all")}async function g(){try{const t=await(await fetch(window.userRoutes.stats,{headers:{"X-CSRF-TOKEN":v,Accept:"application/json"}})).json();e.totalUsers.textContent=t.total,e.activeUsers.textContent=t.active,e.inactiveUsers.textContent=t.inactive}catch(a){console.error("Error loading stats:",a)}}async function p(){try{m=(await(await fetch(window.userRoutes.data,{headers:{"X-CSRF-TOKEN":v,Accept:"application/json"}})).json()).data||[],d=m,y(d),h(d)}catch(a){console.error("Error loading users:",a)}}function A(a){a==="all"?d=m:a==="active"?d=m.filter(t=>t.status==1):a==="inactive"&&(d=m.filter(t=>t.status==0)),d.length===0&&a!=="all"?(e.cardView.classList.add("hidden"),e.tableView.classList.add("hidden"),e.filteredEmptyState.classList.remove("hidden")):(e.cardView.classList.remove("hidden"),e.tableView.classList.remove("hidden"),e.filteredEmptyState.classList.add("hidden"),u==="cards"||u==="grid"||u==="list"?y(d):h(d))}function y(a){if(a.length===0){e.cardView.innerHTML=`
        ...
        `;return}let t="";a.forEach(function(s,i){const o=s.status==1?"active":"inactive",n=s.status==1?"Verified":"Not Verified",r=s.type==1?"Admin":"Staff",c=s.firstname+" "+(s.middlename?s.middlename+" ":"")+s.lastname,b=s.email||'<span style="color: var(--gray);">No email</span>',E=new Date(s.created_at).toLocaleDateString("en-US",{year:"numeric",month:"short",day:"numeric"}),T=s.permissions==0?"Only View button visible":s.permissions==1?"View, Edit, and Delete buttons visible":"View and Edit buttons visible, Delete hidden";t+=`
                ...
                <div class="card-body">
                    ...
                    <div class="user-details">
                        ...
                        <div class="detail-row">
                            ...
                            <span class="detail-value">${B(s.office_id)}</span>
                        </div>
                    </div>
                </div>
        ...
        `,(s=document.getElementById("create_new_empty_table"))==null||s.addEventListener("click",openCreateModal);return}let t="";a.forEach(function(i,o){const n=i.status==1?"active":"inactive",r=i.status==1?"Verified":"Not Verified",c=i.firstname+" "+(i.middlename?i.middlename+" ":"")+i.lastname,b=i.email||'<span style="color: var(--gray);">No email</span>',E=i.permissions==0?"No Access":i.permissions==1?"Can Modify":"Can Print";t+=`
            <tr data-status="${n}" data-id="${i.id}">
