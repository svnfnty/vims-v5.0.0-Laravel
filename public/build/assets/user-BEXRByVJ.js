import{g as c}from"./index-CB87Sc6I.js";import{S as f}from"./sweetalert2.esm.all-BoFtKDfH.js";var C;const _=((C=document.querySelector('meta[name="csrf-token"]'))==null?void 0:C.getAttribute("content"))||"";let u=[],o=[],m="cards",B="create";const e={totalUsers:document.getElementById("totalUsers"),activeUsers:document.getElementById("activeUsers"),inactiveUsers:document.getElementById("inactiveUsers"),subscribedUsers:document.getElementById("subscribedUsers"),cardView:document.getElementById("cardView"),tableView:document.getElementById("tableView"),tableBody:document.getElementById("tableBody"),filteredEmptyState:document.getElementById("filteredEmptyState"),statusFilter:document.getElementById("statusFilter"),userModal:document.getElementById("userModal"),modalOverlay:document.getElementById("modalOverlay"),modalTitle:document.getElementById("modalTitle"),submitBtn:document.getElementById("submitBtn"),userForm:document.getElementById("userForm"),formMethod:document.getElementById("formMethod"),viewOnlyGroup:document.getElementById("viewOnlyGroup"),createdDateDisplay:document.getElementById("createdDateDisplay"),firstname:document.getElementById("firstname"),middlename:document.getElementById("middlename"),lastname:document.getElementById("lastname"),username:document.getElementById("username"),email:document.getElementById("email"),password:document.getElementById("password"),avatar:document.getElementById("avatar"),type:document.getElementById("type"),status:document.getElementById("status"),permissions:document.getElementById("permissions"),credit:document.getElementById("credit"),office:document.getElementById("office"),subscription_type:document.getElementById("subscription_type"),subscription_start_date:document.getElementById("subscription_start_date"),subscription_end_date:document.getElementById("subscription_end_date"),last_payment_date:document.getElementById("last_payment_date"),subscription_amount:document.getElementById("subscription_amount")};function x(a){if(!a)return"N/A";const t=window.offices.find(s=>s.id==a);return t?t.office_name:"N/A"}function g(a="perform this action"){if(window.userPermissions===0)return f.fire({title:"Subscription Required!",text:`Your subscription has expired or you don't have permission to ${a}. Please renew your subscription to continue using this feature.`,icon:"warning",showCancelButton:!0,confirmButtonText:"Renew Subscription",cancelButtonText:"Cancel",confirmButtonColor:"#3085d6",cancelButtonColor:"#d33",allowOutsideClick:!1}).then(t=>{t.isConfirmed&&(window.location.href="/account/setting")}),!1;if(!window.currentUserSubscription||!window.currentUserSubscription.subscription_type)return f.fire({title:"Subscription Required!",text:`You don't have an active subscription. Please subscribe to ${a}.`,icon:"warning",showCancelButton:!0,confirmButtonText:"Subscribe Now",cancelButtonText:"Cancel",confirmButtonColor:"#3085d6",cancelButtonColor:"#d33",allowOutsideClick:!1}).then(t=>{t.isConfirmed&&(window.location.href="/account/setting")}),!1;if(window.currentUserSubscription.subscription_type==="free_trial"){const t=window.currentUserSubscription.subscription_end_date?new Date(window.currentUserSubscription.subscription_end_date):window.currentUserSubscription.last_payment_date?new Date(new Date(window.currentUserSubscription.last_payment_date).setMonth(new Date(window.currentUserSubscription.last_payment_date).getMonth()+1)):null;if(t&&new Date>t)return f.fire({title:"Free Trial Ended!",text:`Your free trial has ended. Please upgrade your subscription to ${a}.`,icon:"warning",showCancelButton:!0,confirmButtonText:"Upgrade Now",cancelButtonText:"Cancel",confirmButtonColor:"#3085d6",cancelButtonColor:"#d33",allowOutsideClick:!1}).then(s=>{s.isConfirmed&&(window.location.href="/account/setting")}),!1}return!0}document.addEventListener("DOMContentLoaded",function(){var a,t,s,i,r,l,p;S(),E(),(a=document.getElementById("loadDataBtn"))==null||a.addEventListener("click",E),(t=document.getElementById("create_new"))==null||t.addEventListener("click",function(n){if(!g("create new users")){n.preventDefault(),n.stopPropagation();return}openCreateModal()}),(s=document.getElementById("resetFilters"))==null||s.addEventListener("click",$),(i=e.subscription_type)==null||i.addEventListener("change",function(){const n=this.value;if(!n){e.subscription_start_date&&(e.subscription_start_date.value=""),e.subscription_end_date&&(e.subscription_end_date.value="");return}const b=new Date().toISOString().split("T")[0];let d=new Date;n==="monthly"?d.setMonth(d.getMonth()+1):n==="yearly"?d.setFullYear(d.getFullYear()+1):n==="free_trial"&&d.setDate(d.getDate()+14);const v=d.toISOString().split("T")[0];e.subscription_start_date&&(e.subscription_start_date.value=b),e.subscription_end_date&&(e.subscription_end_date.value=v),e.last_payment_date&&(e.last_payment_date.value=b)}),document.querySelectorAll(".toggle-btn").forEach(n=>{n.addEventListener("click",function(){m=this.dataset.view,document.querySelectorAll(".toggle-btn").forEach(y=>y.classList.remove("active")),this.classList.add("active"),e.cardView.style.display="none",e.tableView.style.display="none",e.cardView.classList.remove("active","cards-view","grid-view","list-view"),e.cardView.classList.add("users-grid"),m==="cards"?(e.cardView.classList.add("cards-view","active"),e.cardView.style.display="grid",w(o)):m==="grid"?(e.cardView.classList.add("grid-view","active"),e.cardView.style.display="grid",w(o)):m==="list"?(e.cardView.classList.add("list-view","active"),e.cardView.style.display="grid",w(o)):m==="table"&&(e.tableView.classList.add("active"),e.tableView.style.display="block",D(o))})}),(r=e.statusFilter)==null||r.addEventListener("change",function(){T(this.value)}),(l=e.userForm)==null||l.addEventListener("submit",function(n){n.preventDefault(),!e.submitBtn.disabled&&(e.submitBtn.disabled=!0,e.submitBtn.innerHTML='<i class="fas fa-spinner fa-spin"></i> Saving...',V())}),(p=e.modalOverlay)==null||p.addEventListener("click",closeUserModal)});function $(){e.statusFilter.value="all",T("all")}async function S(){try{const t=await(await fetch(window.userRoutes.stats,{headers:{"X-CSRF-TOKEN":_,Accept:"application/json"}})).json();e.totalUsers.textContent=t.total,e.activeUsers.textContent=t.active,e.inactiveUsers.textContent=t.inactive;const s=u.filter(i=>i.subscription_type&&i.subscription_type!=="").length;e.subscribedUsers&&(e.subscribedUsers.textContent=s)}catch(a){console.error("Error loading stats:",a)}}async function E(){try{u=(await(await fetch(window.userRoutes.data,{headers:{"X-CSRF-TOKEN":_,Accept:"application/json"}})).json()).data||[],o=u,w(o),D(o)}catch(a){console.error("Error loading users:",a)}}function T(a){if(a==="all")o=u;else if(a==="active")o=u.filter(t=>t.status==1);else if(a==="inactive")o=u.filter(t=>t.status==0);else if(a==="subscribed")o=u.filter(t=>t.subscription_type&&t.subscription_type!=="");else if(a==="expired"){const t=new Date;o=u.filter(s=>{if(!s.subscription_type)return!1;const i=s.subscription_end_date?new Date(s.subscription_end_date):s.last_payment_date?new Date(new Date(s.last_payment_date).setMonth(new Date(s.last_payment_date).getMonth()+1)):null;return i&&i<t})}o.length===0&&a!=="all"?(e.cardView.classList.add("hidden"),e.tableView.classList.add("hidden"),e.filteredEmptyState.classList.remove("hidden")):(e.cardView.classList.remove("hidden"),e.tableView.classList.remove("hidden"),e.filteredEmptyState.classList.add("hidden"),m==="cards"||m==="grid"||m==="list"?w(o):D(o))}function w(a){if(a.length===0){e.cardView.innerHTML=`
        ...
        `;return}let t="";a.forEach(function(s,i){const r=s.status==1?"active":"inactive",l=s.status==1?"Verified":"Not Verified",p=s.type==1?"Admin":"Staff",n=s.firstname+" "+(s.middlename?s.middlename+" ":"")+s.lastname,y=s.email||'<span style="color: var(--gray);">No email</span>',b=new Date(s.created_at).toLocaleDateString("en-US",{year:"numeric",month:"short",day:"numeric"}),d=s.permissions==0?"Only View button visible":s.permissions==1?"View, Edit, and Delete buttons visible":"View and Edit buttons visible, Delete hidden";let v="No Subscription",h="badge-secondary";if(s.subscription_type){const U=new Date,A=s.subscription_end_date?new Date(s.subscription_end_date):s.last_payment_date?new Date(new Date(s.last_payment_date).setMonth(new Date(s.last_payment_date).getMonth()+1)):null;if(A){const L=Math.ceil((A-U)/864e5);L<0?(v="Expired",h="badge-danger"):L<=7?(v=`Expires in ${L} days`,h="badge-warning"):(v=`Active (${s.subscription_type})`,h="badge-success")}}t+=`
            <div class="user-card" data-status="${r}" data-id="${s.id}">
                <div class="card-header">
                    <div class="user-meta">
                        <span class="user-id">User #${s.id}</span>
                        <span class="user-date">${b}</span>
                    </div>
                ...
                <div class="card-body">
                    ...
                    <div class="user-details">
                        ...
                        <div class="detail-row">
                            ...
                            <span class="detail-value"><span class="badge ${h}">${v}</span></span>
                        </div>
                        <div class="detail-row">
                        ...
                        <div class="detail-row">
                            ...
                            <span class="detail-value">${x(s.office_id)}</span>
                        </div>
                    </div>
                </div>
        ...
        `}),e.cardView.innerHTML=t,I()}function D(a){var s;if(a.length===0){e.tableBody.innerHTML=`
            <tr>
                <td colspan="11">
                    <div class="empty-state">
                        <div class="empty-icon">
        ...
        `,(s=document.getElementById("create_new_empty_table"))==null||s.addEventListener("click",openCreateModal);return}let t="";a.forEach(function(i,r){const l=i.status==1?"active":"inactive",p=i.status==1?"Verified":"Not Verified",n=i.firstname+" "+(i.middlename?i.middlename+" ":"")+i.lastname,y=i.email||'<span style="color: var(--gray);">No email</span>',b=i.permissions==0?"No Access":i.permissions==1?"Can Modify":"Can Print";let d='<span class="badge badge-secondary">No Subscription</span>';i.subscription_badge&&(d=i.subscription_badge),t+=`
            <tr data-status="${l}" data-id="${i.id}">
                ...
                <td class="text-center">
                    <span class="status-badge ${l}">${p}</span>
                </td>
